...
export default function App() {
  ...
  return (
    <>
      ...
      <Modal presentationStyle='pageSheet' visible={showSubscriptionWall} animationType='slide'>
        <View style={{ backgroundColor: '#f7fafb', padding: 30, height: '100%', minHeight: '100%', justifyContent: 'center' }}>
          <View style={{}}>
          ...
          {subscriptions != null && subscriptions.length > 1 ? (
            subscriptions.filter(prod => prod.price > 0).map((product: AdaptyProduct) => (
              <TouchableOpacity key={product.vendorProductId} activeOpacity={1} onPress={() => setActiveSubscription(product)}>
                <View style={(activeSubscription == product) ? (styles.subscriptionSelected) : (styles.subscription)}>
                  <View style={[{ flex: 1, flexDirection: 'column', marginRight: 25, borderRadius: 10, justifyContent: 'center', borderWidth: 1, borderColor: activeSubscription == product ? 'white' : '#2185d0' }]}>
                    <SpecialIcon name='calendar-week' type='font-awesome-5' color={activeSubscription == product ? 'white' : '#2185d0'} />
                  </View>
                  <View style={[{ flex: 3, flexDirection: 'column' }]}>
                    <Text style={{ color: activeSubscription == product ? 'white' : '#2185d0', fontSize: 17, fontWeight: '600', marginBottom: 2 }}>{product.localizedTitle}</Text>
                    <Text style={{ color: activeSubscription == product ? 'white' : '#2185d0', marginBottom: 2 }}>${product.price} / {product.localizedSubscriptionPeriod}</Text>
                    <Text style={{ color: activeSubscription == product ? 'white' : '#2185d0' }}>Paid {product.localizedDescription}</Text>
                  </View>
                </View>
              </TouchableOpacity>
            ))) :
          ...
          <View style={{ flexDirection: 'row', backgroundColor: 'transparent', borderColor: '#2185d0', borderRadius: 5 }}>
            <View style={[{ flexDirection: 'column' }]}>
              <CheckBox wrapperStyle={{ margin: 0, padding: 0 }} onPress={() => setConditionAccepted(!conditionAccepted)} checked={conditionAccepted} containerStyle={{ backgroundColor: 'transparent', borderRadius: 5, borderWidth: 0 }} />
            </View>
          ...
          <Button onPress={() => authContextValue.subScribe(activeSubscription!)} disabled={!conditionAccepted || activeSubscription == undefined} title='Subscribe' containerStyle={{ marginTop: 25 }} buttonStyle={{ paddingVertical: 15, borderRadius: 15 }} />
          <Button onPress={authContextValue.signOut} title='Sign out' containerStyle={{ marginTop: 25 }} titleStyle={{ color: '#2185d0' }} buttonStyle={{ backgroundColor: 'transparent' }} />
        </View>
      </Modal>
    </>
  )
